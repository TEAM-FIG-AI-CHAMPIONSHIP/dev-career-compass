...
sources/ninehire/       나인하이어 3곳 (요기요, 리멤버, 메가존클라우드)
sources/recruiter/      리크루터(Jobflex) 2곳 (GS리테일, 컴투스)
sources/custom/         자체구축 (네이버·카카오·토스·SK그룹 등)
scripts/                감사·집계·분류 진단
docs/                   이 폴더
```
...
**나인하이어:** 요기요, 리멤버, 메가존클라우드

**리크루터:** GS리테일, 컴투스

**자체구축:** 네이버, 카카오뱅크, 라인, 당근, 티빙, 채널톡, 뱅크샐러드, 하이퍼커넥트, 쏘카, 카카오, 토스, SK플래닛(skcareers)
