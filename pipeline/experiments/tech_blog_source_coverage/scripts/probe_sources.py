...
def probe_sitemaps(
    ...
    blog_url = source["blog_url"]
    blog_prefix = blog_url.rstrip("/") + "/"
    urls_under_blog = sum(
        1
        for url in all_urls
        if (
            url.rstrip("/") == blog_url.rstrip("/")
            or url.startswith(blog_prefix)
        )
    )
...
def main():
    ...
    for source in sources:
        ...
        if (
            args.company
            and source["company"] != args.company
        ):
            continue
